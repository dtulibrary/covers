### Modified from 'http://blog.choonkeat.com/weblog/2007/02/rmagick-renderi.html'
...
module RMagickTextUtil
    def render_cropped_text(caption_text, width_constraint, height_constraint,point_size=nil, &block)
        image = render_text(caption_text, width_constraint,point_size, &block)
        if height_constraint < image.rows
            ...
            image = render_text(caption_text[0..end_index] + "...", width_constraint,point_size, &block)
    ...
    def render_text(caption_text, width_constraint,point_size=nil, &block)
        Magick::Image.read("caption:#{caption_text.to_s}") {
            # this wraps the text to fixed width
            self.size = width_constraint
            self.pointsize = point_size if point_size
